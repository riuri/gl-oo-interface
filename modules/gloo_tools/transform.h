...
public:
  ...
  // -> Load/Query methods.
  void LoadIdentity();                      // Sets the top matrix to be identity 4x4.
  void LoadTopMatrix(const glm::mat4 & m);  // Sets the top matrix to be m.
  void LoadTopMatrix(const float* m);       // Sets the top matrix to be m (column-major).
